...
from bot.handlers.personalities import start_personality
...
async def open_personality(callback: CallbackQuery, state: FSMContext):
    await start_personality(callback, state)
    await callback.answer()
